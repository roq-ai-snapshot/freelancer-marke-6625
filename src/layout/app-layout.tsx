...
import { useContractFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['tech-admin', 'business-owner', 'project-manager', 'freelancer', 'client', 'team-member'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useContractFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Contracts',
      path: isTenantUser ? `/contracts/view/${data?.id}` : '/contracts',
      entity: 'contract',
